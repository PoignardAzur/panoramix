...
pub trait VirtualDom<ParentComponentState> {
    ...
    fn init_tree(&self) -> (Self::TargetWidget, Self::DomState);
...
impl<ParentComponentState> VirtualDom<ParentComponentState> for LabelTarget<ParentComponentState> {
    ...
    fn init_tree(&self) -> (Self::TargetWidget, Id) {
        let text = &self.0;
        let id = Id::new();
    ...
    fn apply_diff(&self, _other: &Self, prev_state: Id, widget: &mut Self::TargetWidget) -> Id {
        let text = &self.0;
...
impl<ParentComponentState> VirtualDom<ParentComponentState> for EmptyElementTarget<ParentComponentState> {
    ...
    fn init_tree(&self) -> (Self::TargetWidget, ()) {
        (SingleWidget(WidgetPod::new(druid_w::Flex::row())), ())
    }

    fn apply_diff(&self, _other: &Self, _prev_state: (), _widget: &mut Self::TargetWidget) -> () {}
